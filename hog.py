...
def roll_dice(num_rolls, dice=six_sided):
    ...
    #rolling the dice, and adding all the outcomes
    #if outcome is 1, return 1
    roll = 0
    outcome = 0
    score =0
...
def tail_points(opponent_score):
    ...
    #subtract them, mult by 2, add one --> return result
    decPlace = 1
    currDigit = 0
    diffDigits = 0 #(different of 10s and 1s place)
    while decPlace < 100:
        decPlace *= 10
        currDigit = int((opponent_score % decPlace) / (decPlace / 10))
        diffDigits = currDigit - diffDigits
    return (2 * abs(diffDigits)+1)
...
def play(strategy0, strategy1, update,
    ...
    def strategy(strategy0, strategy1):
        num_rolls = 0
        if who == 0:
            num_rolls = strategy0(player_score(), opp_score())
            return num_rolls
        else:
            num_rolls = strategy1(player_score(), opp_score())
            return num_rolls
    ...
    while score0 < goal and score1 < goal:
        num_turns = strategy(strategy0, strategy1)
        if update == simple_update:
            score = simple_update(num_turns, player_score(), opp_score(), dice)
        else:
            score = square_update(num_turns, player_score(), opp_score(), dice)
        if who == 0:
            score0 = score
        else:
            score1 = score
...
def is_always_roll(strategy, goal=GOAL):
    ...
    s_o = 0
    while s_p <= 100:
        rolls = strategy(s_p, s_o)
        s_p += 1
        if rolls - strategy(s_p, s_o) != 0: 
            t_f = False
            return t_f
        else:
            t_f = True
...
def make_averaged(original_function, total_samples=1000):
    ...
    def ave_func(total_samples, *args):
        sample_num = 0
        f_result = 0
        while sample_num < total_samples:
            sample_num += 1
            f_result += original_function(*args)
        return f_result / total_samples

    return lambda *args : ave_func(total_samples, *args)
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    num_dice = 1
    highest_score = 0
    ave_dice = make_averaged(roll_dice, total_samples)
    while num_dice <= 10:
        score = ave_dice(num_dice, dice)
        if score > highest_score:
            highest_score = score
            dice_max_score = num_dice
        num_dice += 1
    return dice_max_score
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    if square_update(0, score, opponent_score, six_sided) - score >= threshold:
        num_rolls = 0
    else:
        num_rolls = num_rolls
    return num_rolls
