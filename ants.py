...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            elif place.ant.can_contain(other = self):
                    place.ant.store_ant(ant = self)
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        leaf_place = self.place
        leaf_place_move = 0
        while leaf_place.is_hive == False and leaf_place_move < self.lower_bound:
            leaf_place_move += 1
            leaf_place = leaf_place.entrance
        while leaf_place.is_hive == False and leaf_place_move <= self.upper_bound:
                if len(leaf_place.bees) > 0:
                    return random_bee(leaf_place.bees)
                else:
                    leaf_place = leaf_place.entrance
                    leaf_place_move += 1
        return None
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        elif self.turns_to_chew <= self.chewing_turns:
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while behind_queen != None:
            if behind_queen.ant != None:
                behind_queen.ant.double()
...
class SlowThrower(ThrowerAnt):
    ...
    def bee_action_on_odd(self, gamestate):
    ...
    def throw_at(self, target):
        ...
        self.turns_slowed = 0
        target.action = SlowThrower.bee_action_on_odd(self, GameState)
        if target is not None:
            if self.turns_slowed != 5:
                self.turns_slowed += 1
            target.action
    """
    # END Problem EC
